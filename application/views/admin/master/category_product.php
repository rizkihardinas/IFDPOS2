                ...
                <div class="row">
                    <div class="col-md-12">
                    	<div class="form-group">
                            <label for="division_product_id" class="control-label">Division Name</label>
                            <select class="form-control" id="division_product_id">
                                <option value="">-- Choose <?php echo ucwords(str_replace('_', ' ', $this->uri->segment(2))) ?></option>
                                <?php foreach ($division_product_id as $division_product_id): ?>
                                <option value="<?php echo $division_product_id['id'] ?>"><?php echo $division_product_id['name'] ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function(){
                ...
                success:function(data){
                    ...
                    if (json.response == 200) {
                        $.each(json.data, function(key,val){
                            $('#name').val(val.name);
                            $('#division_product_id').val(val.division_product_id);
        ...
        $('#table').DataTable({
            ...
            "ajax":{
                "url": "<?php echo base_url('Api/getAllProductCategory/') ?>",
                "type": "POST"
            }
        });
        ...
        $(document).on('click','#save',function(){
            ...
            var name = $('#name').val();
            var division_product_id = $('#division_product_id').val();
            var value = {
                id:id,
                name:name,
            ...
            Swal.fire({
                ...
                $.ajax({
                    url:'<?php echo base_url('Api/insertCategoryProduct') ?>',
                    type:'POST',
                    data:value,
                    success:function(data){
                        var json = jQuery.parseJSON(data);
