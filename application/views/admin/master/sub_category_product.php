                ...
                <div class="row">
                    <div class="col-md-12">
                    	<div class="form-group">
                            <label for="name" class="control-label">Division Name</label>
                            <select class="form-control" id="category_product_id">
                                <option value="">-- Choose <?php echo ucwords(str_replace('_', ' ', $this->uri->segment(2))) ?></option>
                                <?php foreach ($category_product_id as $category_product_id): ?>
                                <option value="<?php echo $category_product_id['id'] ?>"><?php echo $category_product_id['name'] ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        <div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#table').DataTable({
            ...
            "ajax":{
                "url": "<?php echo base_url('Api/getAllSubProductCategory/') ?>", // URL file untuk proses select datanya
                "type": "POST"
            }
        });
        ...
        $(document).on('click','#save',function(){
            ...
            Swal.fire({
                ...
                $.ajax({
                    url:'<?php echo base_url('Api/insertSubCategoryProduct') ?>',
                    type:'POST',
                    data:value,
                    success:function(data){
                        var json = jQuery.parseJSON(data);
